...
import { getTipiRunState } from '$lib/server/experiments/tipi';
import type { RequestHandler } from './$types';

export const GET: RequestHandler = async ({ cookies, params }) => {
	try {
		const participantSessionId = await requireCookieParticipantConsent(cookies);
		const run = await getTipiRunState(params.runId, participantSessionId);
		...
		return json(
			{ message: experimentSubmissionErrorMessage(error, 'Could not load TIPI run.') },
