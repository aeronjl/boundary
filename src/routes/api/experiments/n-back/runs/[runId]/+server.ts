...
import { getNBackRunState } from '$lib/server/experiments/n-back';
import {
...
export const GET: RequestHandler = async ({ cookies, params }) => {
	try {
		...
		const run = await getNBackRunState(params.runId, participantSessionId);
		...
		return json(
			{ message: experimentSubmissionErrorMessage(error, 'Could not load n-back run.') },
