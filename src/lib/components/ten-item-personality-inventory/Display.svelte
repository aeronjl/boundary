...
<div>
	...
	{#if askingQuestions}
		{#if errorMessage}
			<p class="my-2 font-mono text-xs text-red-600">{errorMessage}</p>
		{/if}
		{#if selectedQuestion}
			<p class="my-2 font-mono text-xs underline">
				Question {trialNumber}{totalTrials ? ` of ${totalTrials}` : ''}
			</p>
		...
		{:else}
			<p class="my-2 font-mono text-xs">Preparing inventory...</p>
		{/if}
		<form class="flex flex-col" on:submit|preventDefault={handleSubmission}>
			<div class="my-4 flex flex-col text-gray-400">
				{#each [{ id: 'disagree-strongly', value: 'Disagree strongly' }, { id: 'disagree-moderately', value: 'Disagree moderately' }, { id: 'disagree-a-little', value: 'Disagree a little' }, { id: 'neither-agree-nor-disagree', value: 'Neither agree nor disagree' }, { id: 'agree-a-little', value: 'Agree a little' }, { id: 'agree-moderately', value: 'Agree moderately' }, { id: 'agree-strongly', value: 'Agree strongly' }] as option (option.id)}
					<div class="flew-row flex items-center gap-2 focus:text-black">
						...
						<label
							...
							option.value
								? 'text-black'
								: 'text-gray-400'}">{option.value}</label
						>
					</div>
				{/each}
			</div>
			<button
				type="submit"
				disabled={disabled || !selectedQuestion || !selectedValue}
				class="my-2 w-1/4 border border-r-8 border-b-8 border-black p-1 font-mono text-xs active:bg-gray-50"
				>Submit</button
			>
		</form>
	{/if}
