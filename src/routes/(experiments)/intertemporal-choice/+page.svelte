<script lang="ts">
	...
	let scenarioPendingId: IntertemporalPolicyScenarioId | null = null;
	...
	async function runPolicyScenario(scenarioId: IntertemporalPolicyScenarioId) {
		if (pending) return;
		...
		try {
			...
			const response = await fetch(
				`/api/experiments/intertemporal-choice/scenarios/${encodeURIComponent(scenarioId)}/runs`,
				{
					method: 'POST',
					headers: studySessionId ? { 'content-type': 'application/json' } : undefined,
					body: studySessionId ? JSON.stringify({ studySessionId }) : undefined
			...
			<div class="border-t border-gray-200 pt-4">
				<div class="flex flex-col gap-1">
					...
					<p class="max-w-2xl text-gray-500">
						Development shortcuts that complete the task through explicit choice policies.
					</p>
				</div>
				<div class="mt-3 grid gap-3 md:grid-cols-2">
					{#each intertemporalPolicyScenarios as scenario (scenario.id)}
						<button
